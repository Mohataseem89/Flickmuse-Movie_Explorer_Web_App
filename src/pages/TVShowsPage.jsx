...
export default function TVShowsPage({ watchlist, handleAddToWatchlist, handleRemoveFromWatchlist }) {
  ...
  return <section className="min-h-[75vh] bg-[#080a0f] py-12 text-white sm:py-16"><div className="mx-auto max-w-[1600px] px-5 sm:px-8">
    ...
    {page === 1 && trending.length > 0 && <section className="mt-12" aria-labelledby="trending-tv-title"><p className="text-xs font-bold uppercase tracking-[0.18em] text-gray-500">This week</p><h2 id="trending-tv-title" className="mt-2 text-2xl font-black">Trending now</h2><div className="movie-row mt-7 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-5 sm:gap-5">{trending.slice(0, 12).map((show) => <div key={show.id} className="w-[42vw] max-w-[190px] shrink-0 snap-start sm:w-[190px]"><MovieCards movie={show} {...cardProps} /></div>)}</div></section>}
